 ...
 * under the License.
 */
var MessageType = require('./message_types');
...
MultiplexedProcessor.prototype.process = function(inp, out) {
  ...
  if (begin.mtype != MessageType.CALL && begin.mtype != MessageType.ONEWAY) {
    throw new TException('TMultiplexedProcessor: Unexpected message type');
  }
  ...
  if (! (sname in this.services)) {
    throw new TException('TMultiplexedProcessor: Unknown service: ' + sname);
  }
